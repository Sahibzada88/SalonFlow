...
## Multi-service appointments, granular staff permissions, icon + dark mode

31. **Multi-service booking**: `app/customer/book/page.tsx` and the owner/
    staff equivalent `app/dashboard/appointments/new/page.tsx` now show
    the services catalog as a checklist (choose one or more) instead of a
    single dropdown - total price/duration update live, and the end time
    is auto-suggested from the combined duration. `service_ids` (an
    array) is sent instead of a single `service_id`. The appointment edit
    page and detail page were updated to match - the detail page now
    lists every service on the appointment (with a running total when
    there's more than one) instead of a single service line.

32. **Two real, pre-existing bugs found and fixed while touching the
    appointment pages**:
    - `app/dashboard/appointments/new/page.tsx` and the `[id]/edit` page
      both defaulted their status field to `'scheduled'` - a status value
      that doesn't exist in the database's check constraint (see backend
      `CHANGES.md` Addendum 1's design note) - so submitting either form
      without manually changing the status dropdown would fail outright.
      Both now default to a real status (`'approved'` for new/`'requested'`
      for edit) and their dropdowns list only real statuses.
    - `app/dashboard/appointments/[id]/page.tsx` and `.../edit/page.tsx`
      each rendered their **own hardcoded copy of the sidebar** (fixed
      position, hardcoded "SalonFlow" text, no theme toggle, none of the
      dark-mode/branding work from earlier) *in addition to* the real
      `Sidebar` component that `app/dashboard/layout.tsx` already wraps
      every `/dashboard/*` page with - meaning both pages showed two
      overlapping sidebars. Removed the duplicate; both pages now render
      as plain content inside the one real layout, same as every other
      dashboard page.

33. **Invoice creation locks one item per service** when opened from an
    appointment with multiple services, instead of one combined lump sum -
    matching the backend's per-service invoice items (`CHANGES.md` #32).

34. **Staff permissions UI**: the staff management page
    (`app/dashboard/staff/page.tsx`) now has a shared
    `PermissionCheckboxes` block (5 permissions: book, approve, bill,
    manage customers, manage services) used in both the "Add Staff" and
    "Edit Staff" dialogs, and the staff table shows an "N / 5 granted"
    summary per row. Previously there was no way to set or even see a
    staff member's permissions from the UI at all - the backend accepted
    a `permissions` dict but the frontend never sent one (silently
    falling back to defaults) and `GET /staff` didn't even return the
    field (see backend `CHANGES.md` #33).

35. **App icon / favicon**: added `app/favicon.ico`,
    `public/icon-192.png`, `public/icon-512.png`, and
    `public/apple-touch-icon.png`, wired up via `metadata.icons` in
    `app/layout.tsx`.

36. **Dark/light mode toggle**: new `components/theme/ThemeToggle.tsx`,
    added to the dashboard sidebar, customer sidebar, the shared
    `AuthShell` (all 5 auth pages), and the landing page header. An inline
    script in `app/layout.tsx` applies the saved (or system-default)
    theme before React hydrates, avoiding a flash of the wrong theme on
    load. Preference persists in `localStorage`.

37. **Dark-mode text/background contrast fix**: enabling the toggle alone
    would have made much of the app unreadable in dark mode, because most
    pages used hardcoded Tailwind colors (`text-stone-900`, `text-stone-500`,
    `bg-white`, `bg-stone-50`/`100`) instead of the theme's semantic
    tokens (`text-foreground`, `text-muted-foreground`, `bg-card`,
    `bg-muted`) - the hardcoded ones don't respond to the `.dark` class at
    all, so dark backgrounds would have paired with near-black text.
    Systematically replaced across all 17 affected files. Status/colored
    badges (e.g. `bg-green-100 text-green-800`) were deliberately left
    as-is - they stay as small, self-contained colored chips in both
    themes, which reads fine either way and avoids a much larger, riskier
    change of adding per-color `dark:` variants everywhere.
