...
class AppointmentCreate(BaseModel):
    ...
    # NEW: an appointment can now cover several services at once (e.g. a
    # haircut + a beard trim in one visit) - service_ids is the real,
    # authoritative field going forward. service_id above is kept only for
    # any older caller that still sends a single service; when service_ids
    # is provided, it takes priority (see request_appointment/
    # upsert_appointment in the SQL, which apply the same precedence).
...
async def create_appointment(appointment_data: AppointmentCreate, ctx: UserContext = Depends(get_current_user)):
    ...
    result = supabase_client.rpc(
        "request_appointment",
        {
            ...
            "p_service_ids": appointment_data.service_ids,
        },
    ).execute()
