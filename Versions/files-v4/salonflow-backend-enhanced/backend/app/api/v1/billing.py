...
async def create_invoice(invoice_data: InvoiceCreate, ctx: UserContext = Depends(_can_bill)):
    ...
    # If this invoice is tied to an appointment, its price(s) are NOT
    # trusted from the client - they're pulled from appointment_services,
    # which snapshots each service's name/price/duration at the moment it
    # was added to the appointment (booking time), and used to build the
    # invoice's line items here, overriding whatever the request
    # submitted. Services are owner-set, so their price shouldn't be
    # something a client-side request can silently change; only discount/
    # tax remain freely editable per invoice. Using the booking-time
    # snapshot (rather than re-querying the live service price) also means
    # the invoice always matches what the customer actually agreed to when
    # they booked, even if the owner changes that service's price later.
    #
    # An appointment can now cover several services (e.g. haircut + beard
    # trim booked together) - every one of them becomes its own line item
    # on THIS ONE invoice, so a multi-service visit still produces a
    # single, combined invoice rather than one per service.
    #
    # A manual invoice with no appointment_id, or an appointment with no
    # services attached at all, still uses the client-submitted items as
    # before.
    items_to_insert = invoice_data.items
    subtotal = invoice_data.subtotal
    if invoice_data.appointment_id:
        appointment_services_resp = (
            supabase_client.table("appointment_services")
            .select("service_name, price")
            .eq("appointment_id", invoice_data.appointment_id)
            .execute()
        )
        if appointment_services_resp.data:
            items_to_insert = [
                InvoiceItemCreate(description=row["service_name"], quantity=1, unit_price=row["price"])
                for row in appointment_services_resp.data
            ]
            subtotal = sum(row["price"] for row in appointment_services_resp.data)
        else:
            # Backward compatibility: an older appointment created before
            # the multi-service feature existed may only have the legacy
            # single service_id column populated, with no
            # appointment_services rows at all. Fall back to that.
