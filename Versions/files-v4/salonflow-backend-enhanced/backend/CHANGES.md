...
## Addendum 9: multi-service appointments + granular staff permissions

31. **Multi-service appointments.** A customer (or staff booking on their
    behalf) can now select several services for one appointment (e.g. a
    haircut + a beard trim in the same visit), instead of being limited to
    exactly one. New table `public.appointment_services` (see
    `database/01_fresh_schema.sql` and the standalone patch
    `04_patch_multi_service_and_permissions.sql`) links an appointment to
    any number of services, snapshotting each service's name/price/
    duration at the moment it's added - so a later price change to that
    service doesn't retroactively alter appointments/invoices that already
    used the old price. `request_appointment()` and `upsert_appointment()`
    both gained an optional `p_service_ids uuid[]` parameter; the legacy
    single `p_service_id` parameter still works unchanged for any old
    caller, and `appointments.service_id` is still populated (with the
    first selected service) for backward compatibility with anything
    still reading it directly. `get_appointments()` now also returns a
    `services` array per appointment alongside the legacy singular fields.

32. **One invoice per appointment, regardless of service count.**
    `POST /billing/invoices` now builds one line item per service on the
    linked appointment (reading the frozen `appointment_services` price
    snapshot, not the live service price - and not the client's
    submitted price, for the same tamper-resistance reasoning as before),
    so a 3-service visit produces a single combined invoice rather than
    needing one invoice per service. Falls back to the old single-service
    lookup for appointments booked before this feature existed and never
    got an `appointment_services` row.

33. **Granular, owner-customizable staff permissions.** The `staff.
    permissions` JSONB column existed before but only had 3 keys
    (`can_book`/`can_approve`/`can_bill`) and no way for staff-mutation
    routes to actually check the two areas owners are often more
    protective of: `customers.py` and `services.py` previously gated their
    POST/PUT/DELETE routes on role alone (`owner` or `staff` - any staff
    member could do anything a customer/service route allowed). Added two
    new permission keys, `can_manage_customers` (default `true`) and
    `can_manage_services` (default `false`, since pricing is the kind of
    thing an owner is more likely to want to keep to themselves), and
    both routers' mutating endpoints now require the matching permission
    via `require_permission(...)` - viewing (GET) stays available to every
    staff member regardless. `PUT /staff/{id}` already accepted an
    arbitrary `permissions` dict from Addendum 7; the gap was that nothing
    actually enforced the two new keys yet, and `GET /staff` didn't even
    return the `permissions` field for the frontend to display.
