...
create table public.staff (
    ...
    position text,
    -- Granular, owner-customizable permissions. Expanded beyond the
    -- original 3 keys (can_book/can_approve/can_bill) to also cover
    -- customer and service management, so an owner can give each staff
    -- member exactly the access they need (e.g. a receptionist who can
    -- book/bill but shouldn't touch service pricing). Any key not present
    -- defaults to false at the application layer (see
    -- UserContext.has_permission in the backend) - these defaults are
    -- just what a brand-new staff member starts with.
    permissions jsonb not null default '{"can_book": true, "can_approve": true, "can_bill": true, "can_manage_customers": true, "can_manage_services": false}'::jsonb,
    is_active boolean not null default true,
    created_by uuid references public.users(id),
    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now()
...
create table public.appointments (
    ...
    -- LEGACY single-service column. Kept (rather than dropped) for
    -- backward compatibility with anything still reading it directly -
    -- but as of the multi-service booking feature, the real source of
    -- truth for "which service(s) is this appointment for" is the new
    -- public.appointment_services table below. New bookings populate
    -- both: this column is set to the FIRST selected service (so any
    -- old code that only ever looked at a single service_id keeps
    -- working unmodified), and appointment_services holds the complete,
    -- authoritative list.
...
-- ============================================================
-- APPOINTMENT SERVICES  (new: one appointment can cover several services)
-- ============================================================
-- Junction table between appointments and services. `price` is a SNAPSHOT
-- of the service's price at the moment it was added to the appointment -
-- not a live lookup - so that if the owner changes a service's price
-- later, appointments/invoices already booked at the old price aren't
-- silently altered. This is exactly the same "freeze the price at time of
-- use" principle invoice_items already followed for the single-service
-- case; multi-service bookings now get the same guarantee per service.
...
begin
    select coalesce(jsonb_agg(jsonb_build_object(
        ...
        -- Legacy singular fields (first linked service, for any code that
        -- hasn't been updated to read the `services` array below) -
        'service_id', a.service_id, 'service_name', coalesce(s.name, ''), 'service_price', s.price,
        -- Authoritative multi-service list: every service on this
        -- appointment, at the price/duration snapshotted when it was
        -- added (see public.appointment_services).
        'services', coalesce((
...
declare
    ...
    v_service_id uuid;
begin
    -- Backward compatible: a caller can still pass a single p_service_id
    -- (old behavior) or the new p_service_ids array (multi-service
    -- booking). If both are omitted the appointment simply has no
    -- service attached, same as before this feature existed.
...
begin
    ...
    -- When an explicit service list is given (edit form re-submits the
    -- full set each time), replace the appointment's services entirely
    -- rather than trying to diff old vs new - simpler and avoids leaving
    -- stale rows behind.
    if p_service_ids is not null then
...
-- appointment_services: same access rule as its parent appointment.
