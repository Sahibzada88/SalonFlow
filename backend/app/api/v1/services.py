...
@router.post("/")
async def create_service(service_data: ServiceCreate, ctx: UserContext = Depends(_owner_or_staff)):
    if not ctx.salon_id:
...
async def update_service(service_id: str, service_data: ServiceUpdate, ctx: UserContext = Depends(_owner_or_staff)):
    if not ctx.salon_id:
...
async def delete_service(service_id: str, ctx: UserContext = Depends(_owner_or_staff)):
    if not ctx.salon_id:
        raise HTTPException(status_code=404, detail="Service not found")
